#!/usr/bin/env python3
"""Generate Práctica 2 (QUIZ4) Listening audio with ElevenLabs. Run from repo root:
    python tools/gen_practice2_audio.py

Reads the scripts directly from listening-quiz.html (const QUIZ4) so the audio
...
Outputs to mp3/P2/<LEVEL>/<name>.mp3, matching the `audio`/`file` paths in QUIZ4.
...
QUIZ_VAR = "QUIZ4"          # Práctica 2
PREFIX   = "P2"             # mp3/P2/<level>/...
...
FEMALE = {"Girl","Woman","Mum","Mia","Anna","Emma","Kate","Lucy","Grace","Elena","Sofia","Lily","Nora","Ruby"}
    ...
    for lev in ["A2","B1","B2","C1"]:
        for a in d[lev]["audios"]:
            ...
            if a.get("paged") and any(q.get("audio") for q in qs):
                # one file per conversation; pair each script with its question's audio path
